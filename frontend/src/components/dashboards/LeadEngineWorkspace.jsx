    ...
    <div className="absolute inset-0 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-5 py-2 border-b border-terminal-border bg-terminal-panel shrink-0">
        <div className="flex items-center gap-1.5 mr-4">
          ...
          <span className="text-[13px] font-semibold text-terminal-text font-heading">Lead Engine</span>
        </div>
        {TABS.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold font-heading transition-all ${
              activeTab === tab.id
